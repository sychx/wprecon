...
func index_body(path_wp_content, index_raw string) (models_finders *[]models.FindersModel)	{
	...
	var regex = regexp.MustCompile(path_wp_content + "/plugins/(.*?)/.*?[.css|.js]")
...
/* This function is a complement of the function: index_body(); It serves to enumerate how plugin versions. */
func index_body_version(models_finders *[]models.FindersModel, path_wp_content, index_raw string) *[]models.FindersModel {
	var regex = regexp.MustCompile(path_wp_content + `/plugins/(.*?)/.*?[css|js].*?ver=(\d{1,2}\.\d{1,2}\.\d{1,3})`)
