...
func PluginsEnumeratePassive() map[string]string {
	...
	rex := regexp.MustCompile(Database.WPContent + "/plugins/(.*?)/.*?[css|js].*?ver=([0-9\\.-]*)")
	...
	for _, plugin := range submatchall {
		...
		Database.OtherInformationsMapString["target.http.plugins.versions"][name] = version
	}

	return Database.OtherInformationsMapString["target.http.plugins.versions"]
}
...
// The plugins will be returned based on this list: Database.OtherInformationsMapString["target.http.plugins.versions"]
...
func PluginsEnumerateAgressive() map[string]string {
	if response := commons.DirectoryPlugins(); response.Raw != "" {
		...
		for _, plugin := range submatchall {
			...
			Database.OtherInformationsMapString["target.http.plugins.versions"][name] = ""
		}
		...
		rex := regexp.MustCompile(Database.WPContent + "/plugins/(.*?)/.*?[css|js]")
		submatchall := rex.FindAllSubmatch([]byte(raw), -1)
	...
	for name := range Database.OtherInformationsMapString["target.http.plugins.versions"] {
		path := Database.WPContent + "/plugins/" + name + "/"
