...
func RootOptionsRun(cmd *cobra.Command, args []string) {
	...
	if confidence := wordpresscheck(); confidence >= 40.0 {
	...
	} else if confidence < 40.0 && confidence > 15.0 && !Database.Force {
		confidenceString := fmt.Sprintf("%.2f%%", confidence)
		...
		printer.Println()
	} else if confidence < 15.0 && !Database.Force {
	...
	if detectionwaf || aggressivemode {
		if waf := security.WAFAggressiveDetection(); waf != nil {
			name := strings.ReplaceAll(waf.URL.Directory, Database.WPContent+"/plugins/", "")
			...
			if importantfile := text.GetOneImportantFile(waf.Raw); strings.Contains(importantfile, "<!-- Avoid the directory listing. -->") && importantfile != "" {
				response := gohttp.SimpleRequest(Database.Target, waf.URL.Directory+importantfile)
				printer.Danger()
				if readme := text.GetVersionStableTag(response.Raw); readme != "" {
					printer.List("Version:", readme).D()
				} else if changelog := text.GetVersionChangelog(response.Raw); changelog != "" {
					printer.List("Version:", changelog).D()
				}
			} else {
				if response := gohttp.SimpleRequest(Database.Target, waf.URL.Directory+"readme.txt"); response.Response.StatusCode == 200 && response.Raw != "" {
					if readme := text.GetVersionStableTag(response.Raw); readme != "" {
						printer.List("Version:", readme).D()
					} else if changelog := text.GetVersionChangelog(response.Raw); changelog != "" {
						printer.List("Version:", changelog).D()
	...
	if Database.Verbose {
		if sitemapResponse := commons.Sitemap(); Database.Verbose {
	...
	if usersEnumerateP, response := enumerate.UsersEnumeratePassive(); len(usersEnumerateP) > 0 && !aggressivemode {
		...
		for _, username := range usersEnumerateP {
			printer.List(username, "("+Database.OtherInformationsString["target.http.users.method"]+")").D()
		}
	...
	if pluginsEnumerateP := enumerate.PluginsEnumeratePassive(); len(pluginsEnumerateP) > 0 && !aggressivemode {
		for plugin, version := range pluginsEnumerateP {
			...
			if version != "" {
				...
				printer.List("Location:", Database.Target+Database.WPContent+"/plugins/"+plugin+"/").D()
	...
	if themesEnumerateP := enumerate.ThemesEnumeratePassive(); len(themesEnumerateP) > 0 && !aggressivemode {
		for theme, version := range themesEnumerateP {
			...
			if version != "" {
				...
				printer.List("Location:", Database.Target+Database.WPContent+"/themes/"+theme+"/").D()
				printer.List("Version:", version).D()
...
func RootOptionsPostRun(cmd *cobra.Command, args []string) {
	...
	if Database.OtherInformationsString["target.http.index.server"] != "" || Database.OtherInformationsString["target.http.index.php.version"] != "" {
		printer.Done("Target information(s):")
		if server := Database.OtherInformationsString["target.http.index.server"]; server != "" {
			printer.List("Server:", server).D()
		}
		if version := Database.OtherInformationsString["target.http.index.php.version"]; version != "" {
			printer.List("PHP Version:", version).Warning()
		}
		if version := Database.OtherInformationsString["target.http.wordpress.version"]; version != "" {
	...
	if len(Database.OtherInformationsSlice["target.http.indexof"]) > 0 {
		printer.Done("Index Of's:")
		for _, indexofs := range Database.OtherInformationsSlice["target.http.indexof"] {
	...
	if status, _ := commons.XMLRPC(); status != "false" {
		...
		printer.List("Location:", Database.Target+"xmlrpc.php").D()
		printer.List("Checked By:", Database.OtherInformationsString["target.http.xmlrpc.php.checkedby"]).D().L()
	}

	if URL := Database.OtherInformationsString["target.http.admin-page"]; URL != "" {
	...
	if raw := Database.OtherInformationsString["target.http.wp-content/uploads.indexof.raw"]; raw != "" {
		if list := extensions.FindBackupFileOrPath(raw); len(list) > 0 {
			printer.Done("File or Path backup:")
			for _, path := range list {
				printer.List(Database.Target + Database.WPContent + "/uploads/" + path).Done()
			}
			printer.Println()
		}
	}
...
func wordpresscheck() float32 {
	...
	for _, payload := range payloads {
		if strings.Contains(Database.OtherInformationsString["target.http.index.raw"], payload) {
			confidence++
		}
	}
